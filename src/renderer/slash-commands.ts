// Slash Command System for Desktop Commander
...
const commands: SlashCommand[] = [
  ...
  },
  {
    ...
    handler: async (_, ctx) => {
      const result = await (ctx.api as any).copilotCompactSession?.();
      if (!result) return { handled: true, message: 'Session compaction not available.' };
      if (result.success) {
  ...
  },
  {
    ...
    handler: async (_, ctx) => {
      ...
      const list = (conversations as Array<{ id: string; title: string; updated_at: number }>).slice(0, 10).map((c, i) =>
  ...
  },
  {
    ...
    args: '[name]',
    handler: async (args, ctx) => {
      const settings = await ctx.api.getSettings() as any;
      if (!args.trim()) {
        return { handled: true, message: `Current model: **${settings.agenticLoop?.model || 'gpt-4o'}**` };
      }
      const newModel = args.trim();
      await ctx.api.setSettings({
        agenticLoop: { ...settings.agenticLoop, model: newModel },
      });
      return { handled: true, message: `Model switched to: **${newModel}**` };
    },
  },
  {
    name: 'settings',
