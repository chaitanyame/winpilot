// URL Fetch Service
// Fetches web pages and returns cleaned text content
 ...
 */
export async function fetchUrl(url: string, maxLength: number = DEFAULT_SUMMARY_LENGTH): Promise<FetchResult> {
  try {
    ...
    if (isPrivateUrl(parsedUrl)) {
      return { success: false, url, error: 'Cannot fetch private/local URLs' };
    }
...
/**
 * HTTP GET with redirect following and timeout
 */
...
/**
 * SSRF protection: check if URL is private/local
 */
